...
//   try {
//     const res = await fetch("http://127.0.0.1:8004/api/config");
...
// waypoint stringni normalize qilish
function normalizeWaypointForUrl(str) {
  if (!str) return "";
...
// Preview uchun waypointlar (string)
function getPreviewWaypoints(data) {
  const wps = Array.isArray(data.waypoints) ? data.waypoints : [];
  return wps
    .slice(0, 20)
    .map((w) => normalizeWaypointForUrl(w))
    .filter(Boolean);
}

// Bitta manzilni geocode qilish (near fallback bilan)
function geocodeAddress(address) {
  return new Promise((resolve) => {
    if (!geocoder || !address) return resolve(null);

    const original = address.toString().trim();

    function tryGeocode(query, isFallback = false) {
      ...
      geocoder.geocode({ address: query }, (results, status) => {
        if (status === "OK" && results[0]) {
          ...
          const m = original.match(/near\s+(.+)$/i);
          if (m && m[1]) {
            const cityQuery = m[1].trim();
            console.warn(
              "Primary geocode failed, trying city-only fallback:",
              cityQuery
            );
            tryGeocode(cityQuery, true);
          } else {
            resolve(null);
          }
        } else {
    ...
    tryGeocode(original, false);
  });
}

// LLM javobidan lat,lng bilan Maps URL yasash (faqat koordinata)
async function buildGoogleMapsDirUrlFromDataAsync(data, fallbackTravelMode) {
  const originStr = (data.origin || "").trim();
  const destStr = (data.destination || "").trim();
  if (!originStr || !destStr) return null;

  const rawWps = getPreviewWaypoints(data);
  const cleanedWps = rawWps
    .map((w) => normalizeWaypointForUrl(w))
    .filter(Boolean);

  if (!geocoder || typeof google === "undefined") {
    console.warn("Geocoder not ready, no Maps URL will be generated.");
    return null;
  }

  const [originLoc, destLoc, ...wpsLoc] = await Promise.all([
    geocodeAddress(originStr),
    geocodeAddress(destStr),
    ...cleanedWps.map((w) => geocodeAddress(w)),
  ]);

  if (!originLoc || !destLoc) {
    console.warn("Origin/destination geocode bo'lmadi, Maps URL yo'q.");
    return null;
  }
  ...
  const wpCoords = wpsLoc
    .filter(Boolean)
    .map((p) => `${p.lat},${p.lng}`);

  if (wpCoords.length) {
    params.set("waypoints", wpCoords.join("|"));
  }
...
function drawRouteOnMap(data, fallbackTravelMode) {
  ...
  const waypointStrings = getPreviewWaypoints(data);
  const waypointObjects = waypointStrings.map((w) => ({
    location: w,
    stopover: true,
  }));
...
async function handleSubmit() {
  ...
  try {
    ...
    // ----- Maps URL -----
    // 1) backend link fallback
    let finalDirUrl = data.google_maps_link || "";

    // 2) coord-based URL bilan ustidan yozishga harakat qilamiz
    try {
      const coordUrl = await buildGoogleMapsDirUrlFromDataAsync(
        data,
        currentTravelMode
      );
      if (coordUrl) {
        finalDirUrl = coordUrl;
      }
    } catch (e) {
      console.warn("Failed to build coord-based URL:", e);
    }

    mapsLink.href = finalDirUrl || "#";
    copyLinkBtn.disabled = !finalDirUrl;
